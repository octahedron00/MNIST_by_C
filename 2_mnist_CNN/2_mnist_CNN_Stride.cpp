...
double x[60006][900], xt[10001][900];
int y[60006], yt[10001];
unsigned char buffer[784];

double w01[16][5][5], w12[16][13][13][32], w23[32][10];				//weight
double b1[16], b2[32], b3[10]; 						//bias
double z1[16][24][24], z2[32], z3[10]; 						//linear sum
double a1[16][13][13], a2[32], a3[10], da1[16][13][13], da2[32], da3[10]; 	//the answer every time
...
int main(){
	...
	n2 = 10000;
	FILE *log = fopen("2_mnist_CNN_Stride.log", "w");
			...
			for(i0=0; i0<16; i0++){
				for(i1=0; i1<13; i1++){
					for(i2=0; i2<13; i2++){
						for(j1=0; j1<5; j1++){
							for(j2=0; j2<5; j2++){
								a1[i0][i1][i2] += x[b][((i1+i1+j1)*28)+i2+i2+j2]*w01[i0][j1][j2];	// (28*28) -> 16*(24*24)
							}
						}
					}
				}
			...
			for(i0=0; i0<16; i0++){
				for(i1=0; i1<13; i1++){
					for(i2=0; i2<13; i2++){
						for(j1=0; j1<5; j1++){
							for(j2=0; j2<5; j2++){
								w01[i0][j1][j2] += x[b][((i1+i1+j1)*28)+i2+i2+j2]*da1[i0][i1][i2];
							}
						}
					}
				}
			...
			for(i0=0; i0<16; i0++){
				for(i1=0; i1<13; i1++){
					for(i2=0; i2<13; i2++){
						for(j1=0; j1<5; j1++){
							for(j2=0; j2<5; j2++){
								a1[i0][i1][i2] += xt[b][((i1+i1+j1)*28)+i2+i2+j2]*w01[i0][j1][j2];	// (28*28) -> 16*(24*24)
							}
						}
					}
				}
